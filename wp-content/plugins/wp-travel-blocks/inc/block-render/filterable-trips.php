    ...
    <div id="wptravel-blocks-filterable-trips" class="wptravel-blocks-filterable-trips <?php echo 'block-id-'.hash( 'sha256', json_encode($attributes) )?>">
        ...
        <div class="wptravel-filterable-trips-filters">
            <?php $i = 1; foreach( $active_terms as $term ) { ?>
                <button class="filter-btn <?php echo $i == 1 ? 'active': '' ;?>" data-filter="<?php echo $term["slug"] ?>"><?php echo $term["name"] ?></button>
            <?php $i++; } ?>
        </div>
        ...
        $j = 1; foreach( $active_terms as $term ) { ?>
            <div id="category-<?php echo esc_attr( $term["slug"] ); ?>" class="wptravel-filterable-trips-grid wp-travel-itinerary-items wptravel-archive-wrapper <?php echo $layout_type ?> grid-view <?php echo $j == 1 ? 'active': '' ;?>">
                <?php 
                    $query = new WP_Query([
